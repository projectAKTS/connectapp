...
void main() {
  ...
  group('Phase 7BE disabled runtime construction plan boundary', () {
    test('boundary artifact exists and safe executor returns inert metadata',
      ...
      expect(
        boundary,
        isA<CallV2DisabledRuntimeConstructionPlanBoundary>(),
      );
    ...
    test('plan boundary is present and no-op while rollout is false', () {
      expect(
        boundary.recordsRuntimeConstructionPlanBoundaryPresent,
        isTrue,
      );
      expect(boundary.recordsNoOpWhileRolloutFalse, isTrue);
      expect(
        boundary.recordsRouteResolverNotCalledWhileRolloutFalse,
        isTrue,
      );
    });

    test('Phase 7BC and Phase 7BD prerequisites remain passing', () {
      expect(boundary.recordsRuntimeConstructionGatePass, isTrue);
      expect(
        boundary.recordsRuntimeConstructionGateHardeningAuditPass,
        isTrue,
      );
      expect(
    ...
    test('production route and screen exposure remain blocked', () {
    ...
    test('runtime construction planning remains blocked and unavailable', () {
      ...
      expect(boundary.recordsRuntimeDependencyAllocationBlocked, isTrue);
      expect(boundary.recordsRuntimeDependencyPreparationBlocked, isTrue);
      expect(boundary.recordsRuntimeCompositionBlocked, isTrue);
      expect(boundary.recordsRuntimeStartBlocked, isTrue);
      expect(boundary.recordsRuntimeUnavailable, isTrue);
      expect(boundary.recordsRuntimeCreationBlocked, isTrue);
    ...
    test('safe debug output contains no sensitive identifiers', () {
      ...
      expect(boundary.toSafeDebugMap()['statusCount'], 39);
      expect(boundary.toSafeDebugMap()['rollbackCount'], 13);
  ...
  group('Phase 7BE source and app wiring isolation', () {
    ...
    test('implementation and config files do not reference the boundary', () {
      const snake = 'call_v2_disabled_runtime_construction_plan_boundary';
      const typeName = 'CallV2DisabledRuntimeConstructionPlanBoundary';
...
String _mainStartupBoundaryArea(String source) {
  ...
  final planIndex = source.indexOf(
    'executeCallV2DisabledRuntimeConstructionPlanBoundarySafely();',
  );
  ...
  expect(planIndex, greaterThan(shellIndex));
  expect(runAppIndex, greaterThan(planIndex));
