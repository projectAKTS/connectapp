...
void main() {
  final recognition = callV2LifecycleObserverRecognition;

  test('lifecycle observer recognition artifact passes', () {
    expect(recognition, isA<CallV2LifecycleObserverRecognition>());
    expect(
    ...
    expect(recognition.recordsNavigatorRecognitionHardeningPass, isTrue);
    expect(recognition.recordsFinalStagedPreRuntimeAuditPass, isTrue);
    expect(recognition.recordsLifecycleObserverHardeningPass, isTrue);
    expect(recognition.recordsLifecycleObserverClosed, isTrue);
    expect(recognition.recordsNoLifecycleRegistration, isTrue);
    expect(recognition.recordsNoAppLifecycleListener, isTrue);
    expect(recognition.recordsNoWidgetsBindingObserver, isTrue);
    expect(recognition.recordsNoLifecycleCallbacksSubscriptions, isTrue);
  ...
  test('recognition source creates no lifecycle navigator backend or RTC hooks',
      () {
    final source = _read(
      'lib/call_v2/integration/call_v2_lifecycle_observer_recognition.dart',
    );
  ...
  test(
      'real app runtime lifecycle platform and config files do not reference recognition',
      () {
    for (final path in <String>[
      ...
      expect(
        source,
        isNot(contains('call_v2_lifecycle_observer_recognition')),
        reason: path,
      );
      expect(
        source,
        isNot(contains('CallV2LifecycleObserverRecognition')),
        reason: path,
      );
      expect(
        source,
  ...
  test('safe debug output contains no sensitive data', () {
    ...
    expect(recognition.toSafeDebugMap()['statusCount'], 29);
    expect(recognition.toSafeDebugMap()['observerRegistered'], isFalse);
    expect(recognition.toSafeDebugMap()['appLifecycleHookCreated'], isFalse);
    expect(recognition.toSafeDebugMap()['bindingObserverAttached'], isFalse);
    expect(recognition.toSafeDebugMap()['callbacksSubscribed'], isFalse);
    expect(recognition.toSafeDebugMap()['asyncHandlesOpened'], isFalse);
