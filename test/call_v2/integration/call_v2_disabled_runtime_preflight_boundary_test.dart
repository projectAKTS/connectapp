...
void main() {
  final boundary = callV2DisabledRuntimePreflightBoundary;

  group('Phase 7BA disabled runtime preflight boundary', () {
    test('boundary artifact exists and safe executor returns inert metadata',
        () {
      final executed = executeCallV2DisabledRuntimePreflightBoundarySafely();

      expect(boundary, isA<CallV2DisabledRuntimePreflightBoundary>());
    ...
    test('runtime preflight is present and no-op while rollout is false', () {
      expect(boundary.recordsRuntimePreflightBoundaryPresent, isTrue);
      expect(boundary.recordsNoOpWhileRolloutFalse, isTrue);
      expect(boundary.recordsRouteResolverNotCalledWhileRolloutFalse, isTrue);
    });

    test('Phase 7AY and Phase 7AZ prerequisites remain passing', () {
      expect(boundary.recordsStartupBoundaryPass, isTrue);
      expect(boundary.recordsStartupBoundaryHardeningAuditPass, isTrue);
      expect(
    ...
    test('production routes screens runtime and startup remain blocked', () {
      ...
      expect(boundary.recordsRuntimeInstanceUnavailable, isTrue);
    ...
    test('safe debug output contains no sensitive identifiers', () {
      ...
      expect(boundary.toSafeDebugMap()['statusCount'], 35);
      expect(boundary.toSafeDebugMap()['rollbackCount'], 10);
  ...
  group('Phase 7BA source and app wiring isolation', () {
    ...
    test('main.dart imports and executes boundary once in required order', () {
      ...
      expect(
          main.indexOf('$shell());'), lessThan(main.indexOf('$touchpoint();')));
      expect(main.indexOf('$touchpoint();'),
          lessThan(main.indexOf('$startup();')));
      expect(
          main.indexOf('$startup();'), lessThan(main.indexOf('$preflight();')));
      expect(
    ...
    test('implementation and config files do not reference the boundary', () {
      const snake = 'call_v2_disabled_runtime_preflight_boundary';
      const typeName = 'CallV2DisabledRuntimePreflightBoundary';
