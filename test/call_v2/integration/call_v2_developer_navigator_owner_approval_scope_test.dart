...
void main() {
  final scope = callV2DeveloperNavigationOwnerApprovalScope;

  test('approval scope artifact records exact Phase 7P approval', () {
    expect(scope, isA<CallV2DeveloperNavigationOwnerApprovalScope>());
    expect(scope.hasHumanApproval, isTrue);
    expect(scope.isNavigationOwnerOnly, isTrue);
    ...
    expect(scope.wiresRealNavigation, isFalse);
    expect(scope.usesAppNavigationKey, isFalse);
    expect(scope.usesGlobalAppKey, isFalse);
    expect(scope.storesWidgetContext, isFalse);
    expect(scope.wiresMaterialRouteTable, isFalse);
  ...
  test('pre-wiring gate default remains blocked but approved scope evaluates',
    ...
    expect(
      ...
      CallV2DeveloperPreWiringScope.navigatorOwnerOnly,
    );
    expect(
  ...
  test('allowed and forbidden files are scoped to future owner only', () {
    ...
    expect(scope.allowedFutureFilesAreNavigationOwnerOnly, isTrue);
  ...
  test('existing navigator owner skeleton remains hard-disabled and inert', () {
    ...
    for (final action in skeleton.actions) {
      final routeName = action == CallV2DeveloperNavigatorAction.dismissFailure
          ? '/call-v2/failure'
          : '/call-v2/audio';
      final decision = skeleton.decideWhileDisabled(
        action: action,
        generation: skeleton.actions.indexOf(action) + 1,
        ownedRouteName: routeName,
  ...
  test('safe debug output contains no sensitive data or route strings', () {
    ...
    expect(scope.toSafeDebugMap()['evidenceCount'], 19);
    expect(scope.toSafeDebugMap()['allowedFutureFileCount'], 2);
    expect(scope.toSafeDebugMap()['forbiddenFutureFileCount'], 17);
  ...
  test('approval source contains no real app or service hooks', () {
  ...
  test('real app boundary files do not reference approval scope', () {
    for (final path in <String>[
      ...
      expect(
        source,
        isNot(contains('call_v2_developer_navigator_owner_approval_scope')),
        reason: path,
      );
      expect(
        source,
        isNot(contains('CallV2DeveloperNavigationOwnerApprovalScope')),
        reason: path,
      );
    }
  });
  ...
  test('rollback preserves one commit disabled routing and V1 protection', () {
    ...
    expect(
      scope.rollback,
      <CallV2DeveloperNavigationOwnerApprovalRollback>[
...
String _approvalSource() {
  return File(
    ...
    'call_v2_developer_navigator_owner_approval_scope.dart',
  ).readAsStringSync();
}
